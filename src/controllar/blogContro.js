...
const {CharacterV,isVString } = require("../validator/validator");
...
try{
    ...
    if (!isVString(title)) return res.status(400).send({ status: false, message: "Please provide valid title" })
    if (!isVString(body)) return res.status(400).send({ status: false, message: "Please provide valid body" })
    if (!isVString(category)) return res.status(400).send({ status: false, message: "Please provide valid category" });

    if (!CharacterV(authorId)) return res.status(400).send({ status: false, message: "Please provide the valid authorid" })
...
try{
    ...
    if (!CharacterV(blogId)) return res.status(400).send({ status: false, message: "please provide the valid blogId " })
    ...
    return res.status(404).send({ status: false, message: "this blog is not exist" })
        ...
        let updateBlog = await blogModel.findOneAndUpdate({ _id: blogId },{$set: {title: title,body: body,category: category,publishedAt: time,isPublished: true,
...
try {
    ...
    if (!CharacterV(data)) return res.status(400).send({ status: false, message: "Please provide the valid blogid" })
